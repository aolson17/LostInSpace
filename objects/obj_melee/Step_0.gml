...
if state != melee_attack && state != melee_stagger && state != melee_die && target != noone{
	if target.x < x{
		move = -1
	}else if target.x > x{
		move = 1
...
if !place_meeting(x,y+1,par_solid) && state != melee_die{ // Apply gravity
	ysp += grav_speed
}else{ // If on ground
